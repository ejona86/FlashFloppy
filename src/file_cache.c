 ...
 *
 * Approach:
 * - Only have one outstanding I/O op at a time.
 * - Before any read, check if the LRU is dirty. If it is dirty, write the LRU
 *   instead of doing the read. This can't be checked just once up-front because
 *   the LRU changes during I/O scheduling.
 * - Readahead is sector-based, for simplicity. The number of sectors is
 *   limited to make sure the readahead data fits in the cache.
 *
 * Scheduler:
 * 1. If I/O in progress, do nothing
 * 2. Check if cur_sector is in the cache and update LRU. If not, read it
 *    (checking if LRU is dirty first).
 * 2. For each priority readahead sector following cur_sector, check if it is
 *    in the cache and update LRU. If not, read it (checking if LRU is dirty
 *    first).
 * 3. If any sector is dirty, scan LRU to find it. Write it.
 * 4. For each low-priority readahead sector following cur_sector, check if it
 *    is in the cache and update LRU. If not, read it (checking if LRU is dirty
 *    first).
 */
