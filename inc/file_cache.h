 ...
 *
 * Constraints that led to design:
 * - For async write-back, it is hard to guarantee the image's buffer will
 *   remain available. Thus, writes require the cache to own the I/O memory
 *   buffer.
 * - fs requires contiguous buffers for multi-sector reads/writes. Thus, group
 *   sectors in memory.
 * - Most images want to write full sectors without any reads. Thus, have a
 *   fast-path for sector-aligned writes, accepting them as soon as memory
 *   permits.
 * - HFE and QD can do partial-sector writes, but they read first.
 *   Small-sectored IMG/DSK can do partial-sector writes, but such disks are at
 *   worst double density so I/O throughput and caching is easy. Thus, optimize
 *   for simplicity by delaying partial-sector writes until the sector is read.
 */
